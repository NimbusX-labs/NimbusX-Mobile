...
-- Secure profile upsert (used by the mobile app)
...
CREATE TABLE IF NOT EXISTS public.chats (
    ...
    members UUID[] NOT NULL, -- Array of user profile IDs
    ...
    unread_count JSONB DEFAULT '{}'::jsonb, -- e.g. {"user_uuid": 0}
    typing JSONB DEFAULT '{}'::jsonb, -- e.g. {"user_uuid": false}
    admins JSONB DEFAULT '{}'::jsonb, -- e.g. {"user_uuid": true}
...
-- Chats Policies
CREATE POLICY "Allow members to read their chats"
    ON public.chats FOR SELECT
    USING (auth.uid() = ANY(members));

CREATE POLICY "Allow members to create chats"
    ON public.chats FOR INSERT
    WITH CHECK (auth.uid() = ANY(members));

CREATE POLICY "Allow members to update chats"
    ON public.chats FOR UPDATE
    USING (auth.uid() = ANY(members))
    WITH CHECK (auth.uid() = ANY(members));

CREATE POLICY "Allow members to delete chats"
...
CREATE TABLE IF NOT EXISTS public.messages (
    ...
    media_url TEXT,
    media_type TEXT CHECK (media_type IN ('image', 'video', 'audio', 'file', 'gif', 'sticker')),
    media_path TEXT, -- Storage path inside bucket (useful for deletions)
...
-- Messages Policies
CREATE POLICY "Allow members to read messages in their chats"
    ON public.messages FOR SELECT
    USING (public.is_chat_member(chat_id, auth.uid()));

CREATE POLICY "Allow members to send messages to their chats"
...
CREATE POLICY "Allow members to update messages in their chats"
...
-- Contacts Policies
CREATE POLICY "Users can read their own contacts list"
    ON public.contacts FOR SELECT
    USING (auth.uid() = user_id);

CREATE POLICY "Users can add contacts to their own list"
    ON public.contacts FOR INSERT
    WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can remove contacts from their own list"
...
CREATE POLICY "Allow users to delete their own status"
...
CREATE POLICY "Allow public access to status-media"
    ON storage.objects FOR SELECT
    USING (bucket_id = 'status-media');

CREATE POLICY "Allow users to upload status-media"
    ON storage.objects FOR INSERT
...
CREATE POLICY "Allow chat members to read chat media"
    ON storage.objects FOR SELECT
    USING (
...
CREATE POLICY "Allow chat members to upload chat media"
    ON storage.objects FOR INSERT
    WITH CHECK (
        bucket_id = 'chat-media' 
        AND public.is_chat_member((storage.foldername(name))[1]::uuid, auth.uid())
